...
import { fetchWithCredentials } from '../hooks/useAuthenticatedFetch';
...
const INFO_URI = 'https://localhost:8080/github/user';
...
interface AuthProviderProps extends TailorExtensionContextValue {
  children: React.ReactNode;
}

export const AuthProvider = ({ children, userContext }: AuthProviderProps) => {
  ...
  useEffect(() => {
    switch (state.name) {
      case 'initializing': {
        ...
        fetchWithCredentials(INFO_URI, userContext, {
      ...
      case 'logging-out': {
        ...
        const controller = new AbortController();
        fetchWithCredentials(LOGOUT_URI, userContext, {
          signal: controller.signal,
          method: 'DELETE',
        }).finally(() => {
          dispatch({
  ...
  return <Box slot="actions">{actions(state)}</Box>;
};
